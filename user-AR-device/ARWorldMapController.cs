...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class ARWorldMapController : MonoBehaviour
    {
        ...
        public static bool worldMap_downloaded = false;
        public static bool worldMap_deserialized = false;
        ...
        IEnumerator Save()
        {
            ...
            {
                Log(string.Format("Session serialpization failed with status {0}", request.status));
                yield break;
            }
        ...
        IEnumerator Load()
        {
            ...
            //Load server world map
            string url_worldmap = "http://" + (insert the IP address of your server) + ":8000/my-session.worldmap";
            using (UnityWebRequest www = UnityWebRequest.Get(url_worldmap))
            {
                ...
                {
                    //Write downloaded map to local storage
                    byte[] worldmap_data = www.downloadHandler.data;
                    var save_file = File.Open(path, FileMode.Create);
                    var writer = new BinaryWriter(save_file);
                    writer.Write(worldmap_data);
                    writer.Close();
                    worldMap_downloaded = true;
        ...
        {
            if (!File.Exists(Application.persistentDataPath + "/my_session.worldmap"))
            {
                Debug.Log("worldmap doesn't exist");
            }

            WWWForm form = new WWWForm();
            form.AddBinaryData("worldmap", File.ReadAllBytes(Application.persistentDataPath + "/my_session.worldmap"), "my_session.worldmap", "application/octet-stream");
            string url_worldmap = "http://" + (insert the IP address of your server) + ":8000/worldmap";
            UnityWebRequest www = UnityWebRequest.Post(url_worldmap, form);
            yield return www.SendWebRequest();
